...
bool ImGui::ColorEdit3(const char* label, unsigned long& col)
{
	const bool value_changed = ImGui::ColorEdit4(label, col, false);
	return value_changed;
}


bool ImGui::ColorEdit4(const char* label, unsigned long& col, bool alpha)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	{
		ImVec4 f = ColorConvertU32ToFloat4(col);
		ImGui::ColorConvertRGBtoHSV(f.x, f.y, f.z, f.x, f.y, f.z);
		i[3] = 255;
		i[0] = IM_F32_TO_INT8(f.x);
		i[1] = IM_F32_TO_INT8(f.y);
		i[2] = IM_F32_TO_INT8(f.z);
	} 
	...
	case ImGuiColorEditMode_HSV:
	{
		...
		const char** fmt = hide_prefix ? fmt_table[0] : (edit_mode == ImGuiColorEditMode_HSV) ? fmt_table[2] : fmt_table[1];
	...
	const ImVec4 col_display = ColorConvertU32ToFloat4(col);
	...
	if (value_changed)
	{
		if (edit_mode == ImGuiColorEditMode_HSV)
		{
			...
			ImGui::ColorConvertHSVtoRGB(i[0] / 255.f, i[1] / 255.f, i[2] / 255.f, r, g, b);

			//i = { 255, IM_F32_TO_INT8(f.x), IM_F32_TO_INT8(f.y), IM_F32_TO_INT8(f.z) };
			i[3] = 255;
			i[0] = IM_F32_TO_INT8(r);
			i[1] = IM_F32_TO_INT8(g);
			i[2] = IM_F32_TO_INT8(b);
		}
		//else
		{
			col = ((i[3] & 0xff) << 24) | ((i[2] & 0xff) << 16) | ((i[1] & 0xff) << 8) | (i[0] & 0xff);
		}
	}
